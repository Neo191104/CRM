...
public class ShiroConfig {
    ...
    public ShiroFilterFactoryBean shiroFilterFactoryBean(SessionsSecurityManager securityManager) throws Exception{
        ...
        shiroFilterChainMap.put("/manager/logout","logout");//此路径的话直接进行登出,回到LoginUrl 指定的页面
        shiroFilterChainMap.put("/manager/**","authc");//此过滤器代表认证过滤器，也就是说，此url必须登录后才能访问
   ...
   /* @Bean
    public ShiroFilterChainDefinition shiroFilterChainDefinition() throws Exception{
        ...
        shiroFilterChainMap.put("manager/logout","logout");//此路径的话直接进行登出,回到LoginUrl 指定的页面
        shiroFilterChainMap.put("/manager/**","authc");//此过滤器代表认证过滤器，也就是说，此url必须登录后才能访问
        shiroFilterChainDefinition.addPathDefinitions(shiroFilterChainMap);
